...
void processSymbolOrders(const std::string& symbol, int numOrders) {
    ...
    for (int i = 0; i < numOrders; ++i) {
        auto order = createRandomOrder(symbol, i + 1000);  // Use deterministic seed
        
        // Use a simple callback that just does console output, nothing fancy
        ...
        if (i % 20 == 0 && i > 0) {  // Simple deterministic approach
            // Cancel an older order
            Order::OrderID idToCancel = nextOrderId.load() - 10;  // Try to cancel an order created 10 orders ago
